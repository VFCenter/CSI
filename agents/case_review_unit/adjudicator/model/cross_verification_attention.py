...
class CrossVerificationAttention(nn.Module):
...
class _CrossVerificationMultiHeadAttention(nn.Module):
    ...
    def forward(self, q, k, v):
        ...
        return q_s, k_s, v_s
class CrossVerificationFeedForward(nn.Module):
...
class CrossVerificationMultiHeadAttention(nn.Module):
    def __init__(self, d_k, d_v, n_heads, dropout, d_model, visual_len, sen_len, fea_v, fea_s, pos):
        super(MultiHeadAttention, self).__init__()
        self.n_heads = n_heads
        self.multihead_attn_v = _CrossVerificationMultiHeadAttention(d_k, d_v, d_model, n_heads, dropout)
        self.multihead_attn_s = _CrossVerificationMultiHeadAttention(d_k, d_v, d_model, n_heads, dropout)
...
class CrossVerificationCoAttention(nn.Module):
    def __init__(self, d_k, d_v, n_heads, dropout, d_model, visual_len, sen_len, fea_v, fea_s, pos):
        super(CrossVerificationCoAttention, self).__init__()
        self.multi_head = CrossVerificationMultiHeadAttention(d_k=d_k, d_v=d_v, n_heads=n_heads, dropout=dropout, d_model=d_model,
                                             visual_len=visual_len, sen_len=sen_len, fea_v=fea_v, fea_s=fea_s, pos=pos)
        self.PoswiseFeedForwardNet_v = CrossVerificationFeedForward(d_model=d_model, d_ff=128, dropout=dropout)
        self.PoswiseFeedForwardNet_s = CrossVerificationFeedForward(d_model=d_model, d_ff=128,dropout=dropout)
    def forward(self, v, s, v_len, s_len):
